...
class Main(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        search_menu.add_command(label = "Part Info", 
                                command = lambda: self.show_frame("SearchPage"))
        search_menu.add_command(label = "Verify Sub")
        search_menu.add_command(label = "List Subs", 
                                command = lambda: self.show_frame("FindSubsPage"))
...
class PurgePage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        purge_button = tk.Button(self, text = "Purge", command = lambda: remove_table(db_var.get().lower()))
    ...
    def __init__(self, parent, controller):
        ...
        def show_part_info():
            ...
            if part_num != "":
                ...
                part_info = {key: value for key, value in part_dict.items() if value != ""}
        ...
        info_search_button = tk.Button(container, text="Search", command=show_part_info)
    ...
    def __init__(self, parent, controller):
        ...
        def make_table(table, part_num, subs):
            ...
            if table == "hdd":
                if part_info["type"] == "HDD":
                    headers = ["Brand", "Part Number", "Type", "Deminsions", 
                            "Height", "Connector", "Capacity (GB)", "Speed", 
                            "Subbed?"]
                    ...
                    headers = ["Brand", "Part Number", "Type", "Height", 
                            "Connector", "HDD Capacity (GB)", "SSD Capacity",
                            "Speed", "Subbed?"]
            elif table == "mem":
            ...
            for col_num in range(0, len(subs[0])):
                ...
                tk.Label(results, text = headers[col_num], width = widths[col_num],
            ...
            for row, sub in enumerate(subs):
                for col, info in enumerate(sub):             
                    ...
                    tk.Entry(results, width = widths[col] + 2, 
                            textvariable = info_var, relief="flat", justify="center",
